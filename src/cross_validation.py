...
class CrossValidation:
    ...
    def split(self):
        if self.problem_type in ["binary_classification", "multiclass_classification"]:
            ...
            print(len(self.dataframe), num_holdout_samples)
...
if __name__ == "__main__":
    # df = pd.read_csv("input/categorical_feature_encoding/train.csv")
    # cv = CrossValidation(df, target_cols=["target"], shuffle=True)

    # df = pd.read_csv("input/house_price/train.csv")
    # cv = CrossValidation(df, target_cols=["SalePrice"], shuffle=True, problem_type="single_col_regression")

    # df = pd.read_csv("input/categorical_feature_encoding/train.csv")
    # cv = CrossValidation(df, target_cols=["target"], shuffle=False, problem_type="holdout_10")

    df = pd.read_csv("input/imet_collection2019/train.csv")
